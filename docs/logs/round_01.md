...
The pipeline generated the following coordinate queries conforming strictly to the requested format ($x_i \in [0,1]$ rounded to six decimal places, separated by hyphens):
...
1. **Low-Dimensional/Unimodal (F1, F2, F3, F5):** If the new entry indicates exit from the zero landscape (a non-trivial signal $y > 0$), the acquisition profile will aggressively shift to **exploitation** ($\beta \rightarrow 0.5$ or switching to Expected Improvement) to climb the localized gradient of the newly discovered peak.
